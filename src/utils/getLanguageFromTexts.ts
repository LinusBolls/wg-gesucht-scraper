...
const getLanguageFromTexts = (texts: string[]) => {
  ...
  const sentences = texts.reduce<string[]>((arr, text) => [...arr, ...text.split(".")], [])

  const detectedLangs = sentences.map((i) => detector.detect(i));

  const occurencesByLang = detectedLangs.reduce<Record<string, number>>((prev, matchPairs) => {
